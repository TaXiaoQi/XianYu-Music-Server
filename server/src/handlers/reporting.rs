...
pub async fn app_open(body: &str, ctx: ReqCtx, pool: &MySqlPool) -> Response {
    ...
    let result = sqlx::query(
        "INSERT INTO app_open_log (device_id, app_version, os_version, device_model, device_name, device_brand, ip, ciyuanxi_id, platform) VALUES (?,?,?,?,?,?,?,?,?)",
    )
    .bind(&device_id)
    .bind(str_of(&data, "app_version"))
    .bind(str_of(&data, "os_version"))
    .bind(str_of(&data, "device_model"))
    .bind(str_of(&data, "device_name"))
    .bind(str_of(&data, "device_brand"))
    .bind(&ctx.client_ip)
